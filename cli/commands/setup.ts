...
export const setupCommand = new Command()
  .description("Initial dotfiles setup — install Nix, clone repo, deploy groups")
  .option("--preset <preset:string>", "Use a named setup preset: core, dev, workstation, ci", {
    default: "core",
  })
  .option("--groups <groups:string>", "Deploy specific groups (comma-separated; overrides preset)")
  ...
  .option("--confirm", "Run with confirmation prompts (default)", { default: true })
  .option("--resume", "Resume from the last failed setup phase or group")
  .option("--from <target:string>", "Resume from a specific phase or deployment group")
  .option("--cwd <path:string>", "Clone dotfiles to PATH (default: ~/Developer/.dotfiles)")
  .action(async (opts) => {
