...
export const rebuildCommand = new Command()
	...
	.action(async (opts: RebuildOptions) => {
		...
		// Prefer the checkout the deployed ~/.config/nix symlinks point into: on
		// deployed machines that is the checkout nix builds from, so updating,
		// evaluating, and compiling all target the same tree even when another
		// checkout (e.g. ~/Developer/.dotfiles) also exists.
		const deployedDir = await resolveDeployedDotfilesDir();
		const dotfilesDir = deployedDir ?? await resolveDotfilesDir();
		const nixConfigDir = await resolveNixConfigDir(dotfilesDir);
		const flakeDir = await Deno.realPath(nixConfigDir);
		const machinePath = await resolveMachineConfigPath(nixConfigDir);

		printInfo(`Using dotfiles checkout: ${dotfilesDir}`);
...
async function updateMachineConfig(machinePath: string, opts: RebuildOptions) {
	...
	if (!await exists(machinePath, { isFile: true })) {
		await generateMachineNix(machinePath);
	}
...
async function generateMachineNix(machinePath: string) {
	printInfo(`machine.nix not found; generating at ${machinePath}`);

	const username = Deno.env.get("USER") ??
		(await captureCommand(["whoami"], Deno.cwd()) || "unknown");
...
# Auto-generated by dotfiles rebuild
...
`,
	...
	printSuccess(
		`Generated machine.nix  Username=${username}  System=${system}  Hostname=${hostname}`,
	);
}
