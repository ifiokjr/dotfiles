...
- `dot clean` - Reclaim disk space: stale AI session transcripts, regenerable caches, Rust `target/` directories (`cargo-clean-all`), nix garbage (`nh clean user`), old rustup toolchains. Dry-run by default, `--apply` deletes. A cache that holds root-owned files (left by a stray `sudo npm install`) is cleared with `sudo rm -rf`, prompting once for a password; the scheduled `--auto` run uses `sudo -n` and never prompts. `dot clean --apply --auto --when-low 100` is what the scheduled disk-space janitor runs (launchd agent on macOS, systemd timer on Linux) whenever free space drops below the threshold
