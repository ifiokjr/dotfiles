...
`PointsPath` is the vector network, and there is no separate network type. Vertices are in the shape's local space, in order, and the four kinds differ only in how they curve:

| Vertex | Properties |
|---|---|
| `StraightVertex` | `x`, `y`, `radius` (corner rounding) |
| `CubicMirroredVertex` | `x`, `y`, `rotation`, `distance` for smooth symmetric handles |
...
For a soft filled glow, use a `RadialGradient` whose outer stop has alpha `00`. That is exactly the technique used for the badge below.
...
`TrimPath` `modeValue` is `sequential` or `synchronized`, and there is no `0`.
...
Scale types for `layoutWidthScaleType` and `layoutHeightScaleType`:

| Value | Meaning |
|---|---|
| `fixed` | Use the `width`/`height` on the component |
| `fill` | Take the space the parent offers. That is a weight, not a percentage |
| `hug` | Shrink to fit the children |

Two traps:

- `undefined` units silently drop the value. Each dimensional property takes a companion `*UnitsValue` accepting `defined|points|percent|auto`. Omitting the attribute is safe: it defaults to `points`, and the value applies. Writing `unitsValue="undefined"` (which an export or a generator can emit) means the value is ignored rather than defaulted. `paddingLeft="16" paddingLeftUnitsValue="undefined"` lays out with no padding at all, and nothing reports it.
...
Meshes deform a raster by moving vertices. The editor auto-traces a contour from the image's alpha; RML cannot, so you author the vertex list yourself.
...
You must supply a font file. Rive ships none and there is no system-font fallback, so a missing font is a hard stop, not a styling problem.
...
`TextStyleBackground` paints a highlighter box behind runs, sized to the glyphs and unioned across them. It is the only box that tracks reflowing text. It takes `cornerRadius` and its own paint.
...
OpenType features work the same way: `smcp` is `1936548720`, `liga` is `1818847073`, `tnum` is `1953396077`. `TextStyleFeature.featureValue` is animatable. A wrong tag integer is indistinguishable from a font lacking the feature, so only a visible A/B proves it works.
