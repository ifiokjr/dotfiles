...
- `duration` is **frames**; `fps` defaults to 60, so two seconds is `duration="120"`.
- `loopValue` is `oneShot` (**the default**), `loop`, or `pingPong`. **An animation with no `loopValue` runs once and holds its final frame** — a continuous spin needs it written explicitly.
...
**Each segment is governed by the keyframe at its start.** The `interpolationType` on the last keyframe is never read — there is no segment after it.
...
- **`cubicValue` shapes the value** — `y1`/`y2` are in the property's own units, which is what the graph editor's curve mode produces. Use it for an overshoot that does not change the timing.
...
**States carry no `name`** — writing one is a build error. They take `stateName`/`stateCaption`, which are editor labels.
...
`duration` is **milliseconds**, unlike animation timing which is frames. Flags: `disabled`, `enableExitTime`, `exitTimeIsPercetange`, `durationIsPercentage`, `pauseOnExit`, `enableEarlyExit`.

**`exitTimeIsPercetange` is misspelled in the format itself.** The correct spelling is not accepted — write the typo.
...
**Every listener under the pointer fires.** Drawing something on top does not block it unless that thing is explicitly opaque (`isTargetOpaque`), and a fully transparent fill is still hit-testable. Two listeners writing the same property will fight — that is the usual reason a control works in one spot and appears dead in another.
...
- `propertyKey` is the key **on the target** — the property being written. `268` is `TextValueRun.text`.
...
| `Interpolator<T>` | — | `ScriptedInterpolator` |
| `Tests` | `function(): Tests` (double wrapper) | — |
...
**A missing `scriptAssetId` link does nothing.** `ScriptedListenerAction` and `ScriptedTransitionCondition` take **no `name`** — adding one fails the build, because they are not `Component`s.
...
- `init(self, context) -> boolean` — called once when the node is created.
- `advance(self, seconds) -> boolean` — per-frame. Returning `true` means "I changed, draw me again".
- `draw(self, renderer)` — render the node. **The former `drawCanvas` callback was removed; move its body into `draw`.**
- `update(self)` — called when an input value changes.
...
**Paths a script builds use the clockwise fill rule**, so when several contours go into one path, winding direction decides fill versus hole — a contour wound the other way is subtracted. That is how a script cuts holes.
...
**If you could draw it in a design tool, build it in RML.** A state machine, a layout, and a text run have no script equivalent — a scripted UI gives up layout, text, and editability to gain nothing.
