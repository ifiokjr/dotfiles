...
`rive docs` and `rive schema` read the copy of the docs shipped beside the binary, so they always match the installed version. That reference is ~9,000 lines and is the real authority — this skill is the workflow around it, and points you at the topic you need.
...
Then **look at the PNG**. `--screenshot` is the only check that has ever caught an appearance bug; `--verify` and `inspect` are structural hygiene and will both pass a screen with an invisible shape, a collapsed icon, or text in the wrong place.
...
1. **Wireframe** — every region as an empty filled box, roughly final size. Give every box a `Fill` (an empty box draws nothing) and size it `fixed`/`fill` (a `hug` box with no children has zero size and vanishes).
2. **Structure** — rows, cells, nested boxes within each region.
...
The object count must go **up** every pass. If it drops, an edit truncated the tree:
...
## Verify animation, not just structure
...
For a time series instead of individual frames, `--data-dump-every` emits JSON Lines — a header, a frame-0 baseline, then only what changed:
...
Compare the three. `rest` and `on` must differ, or the control does nothing. `off` must match `rest`, or it only works one way — the most common state machine bug and invisible to every static check.

- Coordinates are **artboard space**, not window pixels.
...
- **[references/cli-reference.md](references/cli-reference.md)** — every command and flag, exit codes, JSON output shapes, project layout.
- **[references/object-model.md](references/object-model.md)** — RML syntax, the type hierarchy, ids and references, value formats, property keys.
- **[references/artwork.md](references/artwork.md)** — complex artwork: paths, paint, gradients, clipping, meshes, bones, text, layout, components.
- **[references/animation.md](references/animation.md)** — complex animation: timelines, keyframes, easing, state machines, blend states, listeners, view models, Luau.
...
A `.riv` that carries scripts and is destined for the web must be built with `--publish` — unsigned scripts are rejected by the web runtimes, and nothing locally warns you.
