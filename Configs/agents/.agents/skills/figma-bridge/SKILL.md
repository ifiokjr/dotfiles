---
name: figma-bridge
description: Read and write Figma designs from ZCode through the local plugin bridge (mcp__figma-bridge__* tools). Use whenever a task involves changing anything in Figma — editing text or copy in a Figma page, creating/renaming/moving nodes, updating fills, frames or text styles — or when the user says "connect to Figma", "join the Figma channel", "update the Figma file", "sync the copy in Figma", or asks to make Figma changes from the editor. Also use it to bring the bridge up (relay, Figma app, Cursor MCP Plugin) before any Figma work, and to diagnose Figma tool timeouts.
---
...
## Preflight — bring the chain up before any Figma tool call

1. **Relay.** `~/Developer/tools/cursor-talk-to-figma-mcp/zcode-bridge.sh status`.
   If STOPPED, run `... start` (it prints the log path). Never start a second copy.
2. **Figma app.** With computer-use, `list_apps` and look for `com.figma.Desktop`.
   If it is not running, `open_application` with `{"bundle_id": "com.figma.Desktop"}`.
   Figma must have a design file open.
3. **Plugin.** With computer-use, `get_app_state` on the Figma window and look for a
   `Cursor MCP Plugin` element. Take a screenshot (`include_screenshot: true`) and read
   the status line in the plugin window:
   - `Connected to server in channel: <name>` (green) → connected; note the exact name.
   - Button says `Disconnect` → connected; button says `Connect` → click it and wait a
     few seconds, then re-read the channel (it changes on every connect).
   - Plugin window missing → run it: press `cmd+/` in Figma, type `Cursor MCP Plugin`,
...
out while the plugin warms up — wait two seconds and retry once.

## Using the tools

- Tools act on the currently open Figma file; selection-based reads use the current
...
| `Error joining channel: Not connected to Figma` | The MCP server's socket is still opening; retry the join after 2–3 s. |
| `Request to Figma timed out` | The plugin lost its relay connection (its green status can be **stale**). Open the plugin window, click `Disconnect` then `Connect`, re-read the new channel, re-join. |
...
| No `mcp__figma-bridge__*` tools at all | The `figma-bridge` server is missing/disabled in `~/.zcode/cli/config.json` or the session predates it — restart the session; see `references/setup.md`. |
| Relay starts but tools still fail | Confirm port 3055 belongs to the relay: `lsof -nP -iTCP:3055 -sTCP:LISTEN`. |

## Reinstall / first-time setup
