...
1. In the Figma desktop app, open any **Design** file and make sure the toolbelt is in
   Design mode (not Dev Mode, where Figma rejects `editorType: ["figma"]` plugins
...
5. The plugin window shows `Connected to server in channel: <name>`. Keep the window open
   for the whole session, since closing it stops the bridge.
...
`get_figjam`. It is read-only; no channel or plugin needed, but Figma must be running.
