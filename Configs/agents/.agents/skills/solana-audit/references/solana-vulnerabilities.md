...
- Fix: refuse re-init; wipe discriminators on close; Anchor `init` plus owner checks.

### A5. Duplicate / aliased mutable accounts

- Mechanism: the runtime de-duplicates the same account passed in two argument positions (one expected read-only, one mutable), so logic that assumes two distinct accounts misbehaves. In the common case it double-counts or nets to zero what should have been two balances.
...
- Mechanism: sysvars (Clock, Rent, SlotHashes, Instructions) are accounts like any other. A fake "Clock" with a future `unix_timestamp` unlocks vesting, and a spoofed instructions sysvar defeats secp256k1 precompile verification flows.
...
- Evidence: the Wormhole mechanism (Feb 2022). The deprecated `verify_signatures` path trusted a spoofable signature-set account, and the root-cause class is exactly sealevel-attacks #10.
...
- Evidence: Crema ($8.8M: fake tick array account embedding a legit tick address, with provenance never checked).
...
- Mechanism: classic EVM reentrancy is structurally mitigated on Solana (writable accounts are locked across the instruction tree; a program cannot be re-entered with overlapping writable accounts). What remains is state written before a CPI and trusted after it. The callee (attacker-controlled or attacker-influenced) observes or influences intermediate state, or balances read after the CPI reflect attacker actions mid-transaction.
...
This is covered in F1, the ordering trap where an account is closed and its address reused within a flow.
...
- Evidence: Cetus (Sui, May 2025, $223M: u256 to u64 downcast in shared liquidity math; the canonical illustration even off-Solana); Orca Whirlpools (Neodyme 2022-05, Medium "Integer Overflow when swapping"); Phoenix (OtterSec OS-EPS-ADV-03 overflows "during normal operation under certain parameter configurations"); Trail of Bits Wormhole: "Inconsistent use of checked math".
...
- Detect: audit every `a * b / c` in share/price math: upcast to u128 first, round shares down on mint, round assets up on withdrawals and fees (favor the protocol), and handle zero-share/zero-asset branches.
...
- Mechanism: plain `transfer` trusts the source mint's decimals implicitly; `transfer_checked` passes decimals explicitly and validates against the mint, which is required for Token-2022 mints and the safe default everywhere.
...
- Exploit: manipulate the price, borrow or withdraw against the inflated collateral, restore the price (or skip that step), and take the profit, all in one transaction.
...
- Evidence (2026): the Drift April 2026 reports describe a wash-traded fake token whitelisted as collateral. Oracle defense-in-depth must include which asset is priced, as well as how its price is computed.
...
- Cross-ref: `cross-ecosystem.md`, the same math as the EVM ERC-4626 inflation attack.
...
2. **`UncheckedAccount` / bare `AccountInfo`** opts out of all constraints, so the surrounding handler must do signer/owner/type/derivation checks manually. Grep and read every one.
...
4. **`remaining_accounts`** is raw, unconstrained, and positionally indexed, a classic substitution surface. Read every indexing into it.
...
- Mechanism: Solana has no global public mempool, but Jito bundles reintroduce sandwich/backrun MEV on AMM swaps, and transactions visible to validators/relays can be front-run. Priority fees decide ordering through an auction.
...
- Mechanism: transactions stay valid while the recent blockhash is unexpired (~60s/150 blocks), so UX-layer replays must be guarded on-chain. Durable nonces remove expiry entirely: a pre-signed durable-nonce transaction is a live credential that can be submitted weeks later.
- Detect: audit multisig/council signing flows for durable-nonce usage and any "pre-sign for convenience" pattern; treat signed-but-unsubmitted transactions as standing key compromise; require full transaction simulation and content review before signing, and timelocks on admin execution.
...
- SDK/log spoofing: the Phoenix OtterSec audit (High OS-EPS-ADV-01) found the SDK parsed failed transactions as successes. A directly-invoked program can emit fake success logs, so clients must verify transaction status and logs rather than trust what the program emits.
- Key handling: Slope (2022) logged seed phrases to a centralized Sentry server; ~9,000 wallets were drained. Nothing that logs, transmits, or persists key material is in scope-tolerance.
