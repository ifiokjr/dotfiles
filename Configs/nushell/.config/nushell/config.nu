...
def git_main_branch [] {
    let branches = (^git branch --list main master | lines | each { str trim } | where { $in != "" })
    if ("main" in $branches) { "main" } else if ("master" in $branches) { "master" } else { "main" }
}

def git_develop_branch [] {
    let branches = (^git branch --list dev develop development | lines | each { str trim } | where { $in != "" })
...
def gtv [] { ^git tag | lines | sort --natural | reverse }
...
def gunwip [] {
    ...
    if ($msg | str starts-with "--wip--") {
        ^git reset HEAD~1
    } else {
        print "No WIP commit found"
    }
}
