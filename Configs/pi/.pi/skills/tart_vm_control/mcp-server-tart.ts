 ...
 *
 * Standalone MCP server exposing Tart VM operations as tools. Uses vncdo
 * for GUI screenshots and interaction from the host. SSH fallback for commands.
 *
 * Run as an MCP server:
 *   deno run -A /path/to/mcp-server-tart.ts
 *
 * Note: pi users should use the pi extension (.pi/extensions/tart-vm.ts)
 * which is auto-discovered and needs no manual registration.
 *
