...
#
# Merges defaults from ~/.ironclaw.base (managed by tuckr) into
# ~/.ironclaw (which may contain secrets and is not tracked by git).
#
# Strategy:
#   - If ~/.ironclaw does not exist, create it from .ironclaw.base.
#   - If ~/.ironclaw exists, update any keys present in .ironclaw.base
#     with the new default value, preserving keys that only exist in
#     ~/.ironclaw (e.g. secret tokens).
...
IRONCLAW_FILE="$HOME/.ironclaw"
BASE_FILE="$HOME/.ironclaw.base"

if [ ! -f "$BASE_FILE" ]; then
	echo -e "${YELLOW}!${NC} ~/.ironclaw.base not found, skipping ironclaw config generation"
	exit 0
fi
...
if [ ! -f "$IRONCLAW_FILE" ]; then
	...
	echo -e "${GREEN}✓${NC} Created ~/.ironclaw from base defaults"
	echo -e "${BLUE}→${NC} Add secret values (NEARAI_SESSION_TOKEN, NEARAI_API_KEY) to ~/.ironclaw"
else
	...
	echo -e "${GREEN}✓${NC} Merged base defaults into ~/.ironclaw (secrets preserved)"
fi
