...
The `nix` group has a `post_up` hook (`Hooks/nix/post.sh`) that rebuilds the Nix configuration. Setup intentionally runs that hook. In contrast, `dot reload` is a symlink-only operation: it uses `tuckr add --only-files nix` so a reload only reconciles Nix symlinks and cannot rebuild packages or modify the tracked `Configs/nix/.config/nix/flake.lock`. Use `dot rebuild` when package activation is intended.
