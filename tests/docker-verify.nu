...
def main [] {
    ...
    if ($nix_config | path exists) { print $"   PASS: ($nix_config) exists" } else {
    ...
    for pkg in $packages {
        if (which $pkg | is-not-empty) { print $"   PASS: ($pkg) available" } else {
            print $"   FAIL: ($pkg) not found"
            $failures = ($failures | append $"($pkg) not found")
        }
    }
    # 4. Nushell is available
    print "4. Checking nushell..."
    if (which nu | is-not-empty) { print "   PASS: nushell available" } else {
    ...
    if ($scripts_dir | path exists) { print $"   PASS: ($scripts_dir) exists" } else { print $"   WARN: ($scripts_dir) does not exist" }
    ...
    if ($test_script | path exists) {
        ...
        } catch { print "   WARN: test_scripts failed \(non-critical\)" }
    } else {
        ...
        } else { print "   SKIP: test_scripts not found" }
    }
    print ""
    if ($failures | is-empty) { print "All verification checks passed!" } else {
